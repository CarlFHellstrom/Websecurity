    ...
    while True:
        ...
        if not proof[0]:
            ...
            try:
                ...
                text = NODE_PENDING_TRANSACTIONS.decode("utf-8") if isinstance(NODE_PENDING_TRANSACTIONS, bytes) else NODE_PENDING_TRANSACTIONS
                text = text.strip()

                if text:
                    NODE_PENDING_TRANSACTIONS = json.loads(text)
                else:
                    NODE_PENDING_TRANSACTIONS = []
            except Exception:
                # If anything weird comes back (HTML error, etc.), just assume no txs
                NODE_PENDING_TRANSACTIONS = []
...
if __name__ == '__main__':
    ...
    # Create pipe ends for miner <-> web server communication
    pipe_output, pipe_input = Pipe()

    # Start mining in a separate process
    miner_process = Process(
        target=mine,
        args=(pipe_output, BLOCKCHAIN, NODE_PENDING_TRANSACTIONS)
    )
    miner_process.start()

    # Start Flask web server in THIS process on port 5001
    node.run(host='127.0.0.1', port=5001)
