...
def send_transaction(addr_from, private_key, addr_to, amount):
    ...
    if len(private_key) == 64:
        ...
        print(f"\nYour transaction ID is: {message}\n")
    else:
...
def check_transactions():
    ...
    wallet's balance. If the blockchain is too long, it may take some time to load.
    """
    try:
        res = requests.get('http://127.0.0.1:5001/blocks')
    except requests.ConnectionError:
        print('Connection error. Make sure that you have run miner.py in another terminal.')
        return

    if res.status_code != 200:
        print(f"Server returned HTTP {res.status_code}, body was:")
        print(res.text)
        return

    # Try to parse JSON nicely, but don’t crash if it’s not JSON
    try:
        parsed = res.json()
